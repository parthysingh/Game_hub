...
(function () {
  ...
  class PreviewRenderer {
    ...
    drawPreview(ctx, game, w, h) {
      ...
      const centerX = w / 2;
      const centerY = h / 2 + yOff;

      if (game.id === 'pacman') {
        // Draw stylized Pac-Man face
        const radius = 40;
        const mouthOpen = (Math.sin(Date.now() / 200) + 1) / 2; // 0 to 1
        ...
        ctx.beginPath();
        ctx.arc(centerX + radius * 0.2, centerY - radius * 0.5, 5, 0, Math.PI * 2);
        ctx.fill();
      } else {
        // Draw stylized icon (emoji)
        ctx.font = '70px serif';
        ...
        ctx.shadowBlur = 20;
        ctx.shadowColor = game.color;
        ctx.fillText(game.emoji, centerX, centerY);
      }
